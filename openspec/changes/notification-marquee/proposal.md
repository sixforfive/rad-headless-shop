...
The navbar notification sits between `[` `]` brackets with two identical CMS copies already in the item, but the text is static. It should scroll continuously so the banner catches the eye, looping without a visible jump. Empty brackets must not remain when the Notifications collection has no published item.
...
- Webflow markup is already published (collection limit 1, two `.notification-text` nodes, clip on `.notification-holder`). `.notification-bar-box.is-none` already sets `display: none`.

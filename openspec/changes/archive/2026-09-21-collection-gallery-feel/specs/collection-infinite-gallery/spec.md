...
While the viewpoint is moving, smaller tiles SHALL lag behind larger tiles. After pan and coast settle, every tile SHALL sit in the unique packed arrangement (no lasting shear). The lag SHALL stay bounded so copies of the same photo do not drift into each other.

#### Scenario: Size lag during pan

- **WHEN** the visitor drags the gallery
- **THEN** smaller images trail larger ones in the pan direction

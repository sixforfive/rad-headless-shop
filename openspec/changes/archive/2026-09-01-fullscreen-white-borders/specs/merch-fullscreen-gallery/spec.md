...
When the merch gallery is open at any viewport, `body` SHALL have class `is-full-screen`, `color` SHALL be `#fff`, and `border-color` SHALL be `#fff` on `body` and descendants. When the gallery is closed, `body` SHALL NOT have `is-full-screen`, so text color and border color SHALL return to the theme tokens.

...
- [x] 2.3 In `js/cart.js` `onAddToCart`, return when `cartBusy`; lock around `ensureCart` + `addCartLines`; on fail `showCartError` on the clicked `.cta-wrapper`; on success `hideCartErrors` then existing render/open
...
- [x] 3.1 In `css/global.css`, hide `.error-wrapper [id^="error-"]` with opacity 0 and `translateY(0.5rem)`, show `.is-visible` with opacity 1 and no transform, 0.3s ease; do not set position/inset/width
...
- [x] 4.2 In `openspec/specs/shopify-add-to-cart/spec.md`, replace the failed-add requirement with the delta (show mapped error in the clicked `.cta-wrapper`)

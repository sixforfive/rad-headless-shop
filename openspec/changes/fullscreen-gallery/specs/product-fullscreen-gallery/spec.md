...
When `.layer` has class `is-blend`, pointer events SHALL pass through `.layer` except `.navbar`, `.cta-wrapper`, `.footer`, `#close-product`, `#download-spec`, `#add-to-cart`, and `#quantity`, which SHALL receive clicks. `#full-screen-close` SHALL receive clicks while the gallery is open. Clicks on the full-screen image list that are not captured by those controls SHALL close the gallery.
...
- **WHEN** the gallery is open on a viewport wider than 767px and the user clicks `#close-product`, `#download-spec`, `#add-to-cart`, or `#quantity`
- **THEN** that control receives the click and the gallery does not close from that click

#### Scenario: Click through chrome closes

- **WHEN** the gallery is open on a viewport wider than 767px and the user clicks blended chrome that is not `.navbar`, `.cta-wrapper`, `.footer`, `#close-product`, `#download-spec`, `#add-to-cart`, or `#quantity`
